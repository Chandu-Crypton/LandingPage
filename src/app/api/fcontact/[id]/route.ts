...
import FContact from "@/models/FContact";
...
export async function GET(req: NextRequest) {
    ...
    try {
        const doc = await FContact.findById(id);

        if (!doc) {
            return NextResponse.json(
                { success: false, message: 'FContact document not found.' },
        ...
        console.error('GET /api/fcontact error:', error);
        return NextResponse.json(
...
export async function PUT(req: Request) {
    ...
    try {
        ...
        console.error(`PUT /api/fcontact/${id} error:`, error);
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        const deletedDoc = await FContact.findByIdAndDelete(id);
        ...
        console.error(`DELETE /api/fcontact/${id} error:`, error);
        const message = error instanceof Error ? error.message : 'Internal Server Error';
        return NextResponse.json(
            { success: false, message },
            { status: 500, headers: corsHeaders }
