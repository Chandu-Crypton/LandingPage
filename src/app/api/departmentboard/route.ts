...
export async function GET() {
    ...
    try {
        ...
        const docs = await DepartmentBoard.find({});

        if (docs.length === 0) {
            return NextResponse.json(
                { success: true, data: [], message: 'No Department Board documents found.' },
        ...
        console.error('GET /api/departmentboard error:', error);
    ...
    try {
        ...
        if (mainImageFile && mainImageFile.size > 0) {
            ...
            const uploadRes = await imagekit.upload({
                ...
                folder: '/departmentboard_images', // Optional, good for organization
            });
        ...
        const newBlog = await DepartmentBoard.create({
            fullName,
            role,
            socialLink,
            description,
        ...
        return NextResponse.json(
            { success: true, data: newBlog, message: 'Department Board created successfully.' },
            { status: 201, headers: corsHeaders }
        );

    } catch (error) {
        console.error('POST /api/departmentboard error:', error);
        const message = error instanceof Error ? error.message : 'Internal Server Error';
