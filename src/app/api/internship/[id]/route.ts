    ...
    try {
        ...
        const handleNestedArrayWithIcons = async (
            ...
            for (let i = 0; i < parsedArray.length; i++) {
                ...
                if (fieldName === "tool" && (!item.toolTitle || typeof item.toolTitle !== 'string')) {
                    throw new Error(`Invalid tool item at index ${i}: toolTitle is required`);
                }
                if (fieldName === "curriculum" && (!item.currTitle || typeof item.currTitle !== 'string')) {
                    throw new Error(`Invalid curriculum item at index ${i}: currTitle is required`);
                }
                ...
                if (fieldName === "curriculum" && item.weeklyPlan) {
                    if (!Array.isArray(item.weeklyPlan)) {
                        throw new Error(`Invalid curriculum weeklyPlan at index ${i}: must be an array`);
                    }
                    
                    for (const week of item.weeklyPlan) {
                        if (!week.weekTitle || typeof week.weekTitle !== 'string') {
                            throw new Error(`Invalid weeklyPlan item: weekTitle is required`);
                        }
                        if (!Array.isArray(week.topics)) {
                            throw new Error(`Invalid weeklyPlan item: topics must be an array`);
                        }
                    }
                }
