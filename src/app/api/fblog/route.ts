...
export async function GET() {
    ...
    try {
        ...
        const docs = await FBlogModal.find({});

        if (docs.length === 0) {
            return NextResponse.json(
                { success: true, data: [], message: 'No Fetch Blog documents found.' },
        ...
        console.error('GET /api/fblog error:', error);
    ...
    try {
        ...
        if (itemsString) {
            try {
                ...
                return NextResponse.json(
                    { success: false, message: 'Invalid format for fetch blog items.' },
                    { status: 400, headers: corsHeaders }
                );
            }
        }
        ...
        if (mainImageFile && mainImageFile.size > 0) {
            ...
            const uploadRes = await imagekit.upload({
                ...
                folder: '/fblog_images', // Optional, good for organization
            });
        ...
        const newBlog = await FBlogModal.create({
            addHeading,
            blogHeading,
            title,
            tags,
        ...
        console.error('POST /api/fblog error:', error);
        const message = error instanceof Error ? error.message : 'Internal Server Error';
