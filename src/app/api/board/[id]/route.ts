...
export async function PUT(req: NextRequest) {
    ...
    try {
        ...
        const updateData: Partial<IBoard> = {}; // Partial<IBoard>
        ...
        const updatedInternship = await Board.findByIdAndUpdate(
            id,
            { $set: updateData },
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        const deletedDoc = await Board.findByIdAndDelete(id);

        if (!deletedDoc) {
            return NextResponse.json(
                { success: false, message: 'Board document not found for deletion.' },
                { status: 404, headers: corsHeaders }
            );
        }
        ...
        console.error(`DELETE /api/board/${id} error:`, error);
        const message = error instanceof Error ? error.message : 'Internal Server Error';
        return NextResponse.json(
            { success: false, message },
            { status: 500, headers: corsHeaders }
