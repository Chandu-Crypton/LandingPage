...
const TermsConditionsPage = dynamic(() => import('@/components/policycomponents/TermsConditionsComponent'), {
  ssr: false,
  loading: () => (
...
const AdminAboutUsManagementPage: React.FC = () => {
  ...
  const fetchAboutUsContent = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('/api/termscondition'); // Use axios
  ...
  const handleSaveAboutUs = async (dataToSave: { _id?: string; content: string }) => {
    ...
    try {
      ...
      if (dataToSave._id) {
        ...
        response = await axios.put(`/api/termscondition/${dataToSave._id}`, { content: dataToSave.content });
      } else {
        // Create new entry
        response = await axios.post('/api/termscondition', { content: dataToSave.content });
      }
  ...
  const handleDeleteClick = async (id: string) => {
    ...
    setError(null);
    try {
      const response = await axios.delete(`/api/termscondition/${id}`);
      if (response.data.success) {
