...
const NewsLetterListPage: React.FC = () => {
  ...
  const fetchNewsletters = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/fnewsletter');
      if (res.data.success) {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Are you sure you want to delete this fnewsletter?')) return;
    try {
      await axios.delete(`/api/fnewsletter/${id}`);
      fetchNewsletters();
    } catch {
      setError('Failed to delete fnewsletter.');
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-center">FNewsletters List</h1>
                  ...
                  <tr key={newsletter._id} className="border-t hover:bg-gray-50 transition">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex justify-center gap-2">
                        <Link
                          href={`/fnewsletter-management/NewsLetter-List/${newsletter._id}`}
