...
const NewsLetterListPage: React.FC = () => {
  const [newsletters, setNewsletters] = useState<Subscribe[]>([]);
  ...
  const filteredNewsletters = useMemo(() => {
    ...
    return newsletters.filter((newsletter) =>
      newsletter.email.toLowerCase().includes(lowercasedSearchTerm) 
    );
  }, [newsletters, searchTerm]);

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-center">Subscribe List</h1>
      ...
      {/* Newsletters Table */}
      <ComponentCard title="All Subscribers List">
        {!loading ? (
              ...
              <tbody>
                {filteredNewsletters.map((newsletter) => (
                  <tr key={newsletter._id} className="border-t hover:bg-gray-50 transition">
                    <td className="px-5 py-3 font-semibold">{newsletter.email}</td>
