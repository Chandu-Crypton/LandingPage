...
import { EyeIcon, CalendarIcon, PencilIcon } from 'lucide-react';
...
const ContactListPage: React.FC = () => {
    const [contactData, setContactData] = useState<IDisplayContact[]>([]);
    ...
    const fetchContactData = async () => {
        setLoading(true);
        try {
            const res = await axios.get('/api/displaycontact');
            if (res.data.success) {
    ...
    // Date filter
    const filterByDate = (contacts: IDisplayContact[]) => {
    ...
    const filteredContacts = useMemo(() => {
        ...
        return result.filter((c) => {
            return (
                ...
                (c.timings?.toLowerCase() || '').includes(lower)
            );
        });
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 sticky right-0 bg-white shadow-md z-10">
                                                <div className="flex justify-center gap-2">
                                                    <Link
                                                        href={`/displaycontact-management/DisplayContact-List/${entry._id}`}
                                                    ...
                                                    <Link
                                                        href={`/displaycontact-management/Add-DisplayContact?page=edit&id=${entry._id as string}`}
