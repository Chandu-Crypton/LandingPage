...
//   // Combine search + date filters
  ...
  //       (c.interested?.some(interest => interest.toLowerCase().includes(lower)) || false) ||
...
const ContactListPage: React.FC = () => {
  ...
  // Filter functions omitted for brevity (same as your code)
  const filterByDate = (contacts: IContact[]) => {
    if (dateFilter === 'all') return contacts;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {error && <p className="bg-red-100 text-red-700 px-4 py-2 mb-4 rounded-md text-center">{error}</p>}
