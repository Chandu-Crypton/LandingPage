'use client';

import React, { useEffect, useState } from 'react';
import axios from 'axios';
import { ArrowUpIcon, TrashBinIcon, UserIcon } from '@/icons';
import ComponentCard from '@/components/common/ComponentCard';
import StatCard from '@/components/common/StatCard';
import { EyeIcon, PencilIcon } from 'lucide-react';
import Link from 'next/link';



interface IContact {
  _id: string;
  fullName: string;
  email: string;
  phoneNumber: string;
  message: string;
}

const ContactListPage: React.FC = () => {
    const [contactData, setContactData] = useState<IContact[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    // Function to fetch contact data
    const fetchContactData = async () => {
        setLoading(true);
        try {
            const res = await axios.get('/api/contact'); // Assuming your GET endpoint for contact data is /api/contact
            if (res.data.success) {
                setContactData(res.data.data);
            } else {
                setError(res.data.message || 'Failed to load contact data.');
            }
        } catch (err) { // Type the catch error for better handling
            console.error('Error fetching contact data:', err);
            setError( 'Failed to load contact data. Please try again.');
        } finally {
            setLoading(false);
        }
    };

    // Fetch data on component mount
    useEffect(() => {
        fetchContactData();
    }, []);

    // Handle delete operation
    const handleDelete = async (id: string) => {
        if (!window.confirm('Are you sure you want to delete this contact entry?')) return;
        try {
            await axios.delete(`/api/contact/${id}`); // Assuming your DELETE endpoint is /api/contact/:id
            alert('Contact entry deleted successfully!');
            fetchContactData(); // Re-fetch data to update the list
        } catch (err) { // Type the catch error
            console.error('Error deleting contact entry:', err);
            setError( 'Failed to delete contact entry. Please try again.');
        }
    };

    return (
        <div className="container mx-auto px-4 py-8">
            <h1 className="text-3xl font-bold mb-6 text-center">Contact Data</h1>

            {error && (
                <p className="bg-red-100 text-red-700 px-4 py-2 mb-4 rounded">
                    {error}
                </p>
            )}

            {/* Stat Card for total entries */}
            <div className="flex justify-end mb-8">
                <div className="w-full lg:w-1/4">
                    <StatCard
                        title="Total Contact Entries"
                        value={contactData.length}
                        icon={UserIcon} // Using UserIcon as a generic icon, replace if you have a more relevant one
                        badgeColor="success"
                        badgeValue="0.00%" // Placeholder, as percentage change might not be relevant for footer
                        badgeIcon={ArrowUpIcon} // Placeholder
                    />
                </div>
            </div>

            {/* Contact Data Table */}
            <ComponentCard title="Contact Details">
                {!loading ? (
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                            <thead>
                                <tr className="text-gray-600">
                                    <th className="px-5 py-3 text-left">Phone Number</th>
                                    <th className="px-5 py-3 text-left">Full Name</th>
                                    <th className="px-5 py-3 text-left">Email</th>
                                    <th className="px-5 py-3 text-left">Message</th>
                                    <th className="px-5 py-3 text-center">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {contactData.map((entry) => (
                                    <tr key={entry._id} className="border-t hover:bg-gray-50 transition">
                                        <td className="px-5 py-3 font-semibold">{entry.phoneNumber}</td>
                                        <td className="px-5 py-3">{entry.fullName}</td>
                                        <td className="px-5 py-3">{entry.email}</td>
                                        <td className="px-5 py-3">{entry.message}</td>
                                       
                                        <td className="px-5 py-3">
                                            <div className="flex justify-center gap-2">
                                              
                                                  <Link
                                                    href={`/contact-management/Contact-List/${entry._id}`} // Adjust this path as per your routing
                                                    className="text-blue-500 border border-blue-500 rounded-md p-2 hover:bg-blue-500 hover:text-white"
                                                >
                                                    <EyeIcon size={16} />
                                                </Link>
                                                <Link
                                                    href={`/contact-management/Add-Contact?page=edit&id=/${entry._id}`} // Adjust this path as per your routing
                                                    className="text-yellow-500 border border-yellow-500 rounded-md p-2 hover:bg-yellow-500 hover:text-white"
                                                >
                                                    <PencilIcon size={16} />
                                                </Link>
                                                <button
                                                    onClick={() => handleDelete(entry._id)}
                                                    className="text-red-500 border border-red-500 rounded-md p-2 hover:bg-red-500 hover:text-white"
                                                >
                                                    <TrashBinIcon />
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                                {contactData.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="px-5 py-10 text-center text-gray-500">
                                            No contact entries found.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                ) : (
                    <p className="text-gray-600">Loading contact data...</p>
                )}
            </ComponentCard>
        </div>
    );
};

export default ContactListPage;
