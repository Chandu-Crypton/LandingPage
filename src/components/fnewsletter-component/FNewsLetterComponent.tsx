...
import { useFNewsLetter } from '@/context/FNewsLetterContext';
...
const NewsLetterForm: React.FC<FooterFormProps> = ({ newsLetterIdToEdit }) => {
    ...
    const { addNewsLetter, updateNewsLetter, newsletters } = useFNewsLetter();
    ...
    useEffect(() => {
        ...
        if (newsLetterIdToEdit && newsletters.length > 0) {
            ...
            const fetchSingleNewsLetter = async () => {
                const cleanId = newsLetterIdToEdit?.replace(/^\//, "") || '';
                try {
                    const res = await fetch(`/api/fnewsletter/${cleanId}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (newsLetterIdToEdit) {
                ...
                alert('FNewsletter updated successfully!');
            } else {
                await addNewsLetter(newsLetterData);
                alert('Newsletter added successfully!');
                clearForm();
            }
            router.push('/fnewsletter-management/NewsLetter-List');
        } catch (err: unknown) {
