...
import { useSalesContact } from '@/context/SalesContactContext';
...
const ContactForm: React.FC<ContactFormProps> = ({ contactIdToEdit }) => {
  ...
  const { addContact, updateContact, salecontacts } = useSalesContact();
  ...
  useEffect(() => {
    if (contactIdToEdit && salecontacts.length > 0) {
      const cleanId = contactIdToEdit.replace(/^\//, '');
      const contactToEdit = salecontacts.find((f) => f._id === cleanId);
      ...
      if (contactToEdit) {
        ...
        const fetchSingleContact = async () => {
          setLoading(true);
          try {
            const res = await fetch(`/api/salescontact/${cleanId}`);
            const data = await res.json();
            if (res.ok && data.success && data.data) {
              const fetched: ISalesContact = data.data;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/salescontact-management/SaleContact-List');
    } catch (err) {
